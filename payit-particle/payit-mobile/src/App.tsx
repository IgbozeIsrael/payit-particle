...
// Initialize Magic SDK with publishable key
...
function MagicLinkScreen({ onSent, onBack }: { onSent: () => void; onBack: () => void }) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      // Attempt SDK login if available, otherwise provision via backend endpoint
      const magicInstance = initMagic();
      if (magicInstance) {
        try {
          // Trigger Magic Link email dispatch (background promise)
          magicInstance.auth.loginWithMagicLink({ 
            email: cleanEmail,
            showUI: true 
          }).catch(err => console.warn('[Magic] SDK login promise warning:', err));
          console.log('[Magic] Magic link login initiated');
        } catch (magicErr: any) {
          console.warn('[Magic] SDK non-fatal error:', magicErr);
        }
      }
      ...
      try {
        ...
        await fetch(primaryUrl, {
          ...
          // Backend notification is optional - magic link already sent
          console.log('[Magic] Backend notification skipped (not critical)');
        });
      } catch (_) {
        // Ignore backend errors - magic link was already sent by client SDK
      } finally {
        clearTimeout(timeoutId);
      }
...
function MagicVerifyScreen({ onVerified, onBack }: {
  ...
  async function handleStartPinVerification() {
    ...
    try {
      ...
      console.log('[Magic] Checking authentication status...');
      
      // Try to get token - Magic will handle the login check internally
      let token = null;
      let retries = 3;
      ...
      while (retries > 0 && !token) {
        try {
          ...
          if (!isLoggedIn) {
            if (retries === 3) {
              // First attempt - user might not have clicked yet
              setError('Please click the magic link in your email first, then try again.');
              setLoading(false);
              return;
            }
          ...
          token = await magicInstance.user.getIdToken();
          if (token) break;
          
        } catch (err) {
          console.error('[Magic] Token retrieval attempt failed:', err);
          retries--;
      ...
      if (!token) {
        throw new Error('Unable to retrieve authentication token. Please try logging in again.');
      }

      console.log('[Magic] DID token received successfully');
      setDidToken(token);
      sessionStorage.setItem('payit_token', token);

      // Check if user exists and needs PIN
      console.log('[Magic] Fetching user profile...');
      const res = await fetch(`${API}/api/mobile/me`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      
      console.log('[Magic] Profile response status:', res.status);
      
      if (res.ok) {
        const data = await res.json();
        console.log('[Magic] Profile data:', data);
      ...
      // New user - setup PIN
      console.log('[Magic] New user detected, setting up PIN...');
      setPinMode('setup');
      setShowPinInput(true);
      ...
      if (err.message?.includes('timed out') || err.message?.includes('timeout')) {
        setError('Authentication timed out. Please click the magic link in your email.');
      ...
      } else if (err.message?.includes('Unable to retrieve')) {
    ...
    <div className="flex-1 flex flex-col px-5 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col justify-center gap-6">
        <div>
          ...
          <h2 className="text-[24px] font-extrabold text-[#0F172A]">Magic Link Sent!</h2>
          <p className="text-[13px] text-[#64748B] mt-1.5 leading-relaxed">
            We sent a login verification link to <strong className="text-[#0F172A]">{email}</strong>.
            Click below to verify identity and enter your Security Key.
          </p>
        </div>

        <div className="p-4 bg-[#F8FAFC] border border-[#E2E8F0] rounded-[16px] space-y-2">
          <div className="flex items-center gap-2 text-xs font-semibold text-[#0F172A]">
            <Shield size={14} color={FOREST} />
            <span>Multi-Factor Security Active</span>
          </div>
          <p className="text-[11px] text-[#64748B] leading-relaxed">
            Your account is protected by Magic DID token authentication + 6-digit Security PIN authorization.
          </p>
        </div>
      </div>

      <div className="shrink-0 pb-4">
        <GreenBtn label="Verify & Enter Security Key" onClick={handleStartPinVerification} loading={loading} icon={<CheckCircle2 size={16} />} />
      </div>
    </div>
  );
}
